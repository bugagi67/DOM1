...
console.log(data.map(function (item) {
  return {
    idType: _typeof(item.id),
    imdbType: _typeof(item.imdb),
    yearType: _typeof(item.year)
  };
}));
function main() {
  ...
  showTable(data);
  var sortAttr = ["id", "title", "imdb", "year"];
  var counter = 0;
  setInterval(function () {
    var attr = sortAttr[counter % sortAttr.length];
    var increasing = counter % 2 === 0;
